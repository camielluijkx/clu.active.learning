...
namespace clu.active.learning
{
    ...
    The requirements of your system, and how you intend to transport the data, will influence the 
    serialization format you choose. The following table describes some of the common formats 
    available.

    Format          Description
    Binary          Serializing data into the machine-readable binary format enables you to 
                    preserve the fidelity and state of an object between different instances of 
                    your application. Binary serialization is also fast and lightweight, because 
                    the binary format does not require the processing and storage of unnecessary 
                    formatting constructs. Binary serialization is commonly used when persisting 
                    and transporting objects between applications running on the same platform.
    XML             Serializing data into the XML format enables you to utilize an open standard 
                    that can be processed by any application, regardless of platform. In contrast 
                    to binary, XML does not preserve type fidelity; it only lets you serialize 
                    public members that your type exposes. The XML format is more verbose than 
                    binary, as the serialized data is formatted with XML constructs. This makes the 
                    XML format less efficient and more processor intensive during the serializing, 
                    deserializing, and transporting processes. However, the nature of XML as a 
                    plain text and free-form language allows messages to be transmitted across 
                    different applications and platforms. So long as the transmitter and receiver 
                    have agreed on a known contract, both can send and receive messages and convert 
                    them to the appropriate model within their respective environments. XML 
                    serialization was commonly used to serialize data that can be transported via 
                    the SOAP protocol to and from web services. However, due to SOAP’s verbose and 
                    strict nature, this protocol has fallen out of favor, and is generally found 
                    today only in legacy environments.
    JSON            Serializing data into the JSON format enables you to utilize a lightweight, 
                    data-interchange format that is based on a subset of the JavaScript programming 
                    language. JSON is a simple text format that is human readable and also easy to 
                    parse by machine, irrespective of platform. JSON shares XML strengths as plain 
                    text and free form, making it cross platform. However, unlike XML, it has a 
                    much more concise syntax, which makes it cheaper to transmit and human 
                    readable. These advantages made JSON the prevalent language to transmit data 
                    today, and the de-facto standard in today’s industry. JSON is commonly used to 
                    transport data between everything from Asynchronous JavaScript and XML (AJAX) 
                    calls to messages between webservices, because unlike SOAP, you are not limited 
                    to just communicating within the same domain.

    Alternatively, if you want to serialize your data to a format that the .NET Framework does not 
    natively support, you can implement your own custom serializer class.
    ...
    public static class Serialization
    {
        ...
        public class IniFormatter : IFormatter
        {
            ...
            public object Deserialize(Stream serializationStream)
            {
                ...
                MemberInfo[] members = FormatterServices.GetSerializableMembers(
                    obj.GetType(), this.Context);
                ...
                return FormatterServices.PopulateObjectMembers(
                   obj, members, dataAsCorrectTypes.ToArray());
            }
        ...
        public static void UsingSerialization()
        {
            Console.WriteLine("* Using Serialization");
            {
                ...
                Console.WriteLine("** Deserialize an Object using the BinaryFormatter");
                {
                    ...
                    ServiceConfiguration config = formatter.Deserialize(buffer) 
                ...
                Console.WriteLine("** Serialize an Object using the DataContractJsonSerializer");
                {
                    ...
                    // Create a DataContractJsonSerializer object that you will use to serialize 
                    // the object to JSON. DataContractJsonSerializer class is derived from the 
                    // abstract XmlObjectSerializer class, and it is not an implementation of the 
                ...
                {
                    // Create a DataContractJsonSerializer object that you will use to deserialize 
                    // the JSON.
                    DataContractJsonSerializer jsonSerializer
                ...
                Console.WriteLine("** Serialize an Object using the JsonConvert");
                {
                    ...
                    Unlike the .NET implementations of the IFormatter interface, JsonConvert 
                    doesn’t require the data class to be decorated with the Serializable attribute. 
                    Any class can be converted to JSON without special treatment. However, Json.NET 
                    does provide a set of attributes to specify exactly how the model will be 
                    serialized, if necessary. The most useful of them is perhaps the JsonProperty 
                    attribute, which allows to determine the name of the property in the serialized 
                    JSON string.
                ...
                {
                    // Get the JSON string – Here we’re assuming it’s the same from the previous 
                    // example.
                    ServiceConfiguration config = ServiceConfiguration.Default;
                    var jsonString = JsonConvert.SerializeObject(config);

                    // Deserialize to the desired type.
                    var deserializedConfig = JsonConvert
                        .DeserializeObject<ServiceConfiguration>(jsonString);
                }
                ...
                Console.WriteLine("Deserialize an Object using the JsonSerializer");
                {
                    ...
                    // Deserialize to the desired type.
                    var deserializedConfig = serializer
                        .Deserialize<ServiceConfiguration>(jsonReader);
