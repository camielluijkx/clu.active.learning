...
namespace clu.active.learning
{
    ...
    whereas others may be unmanaged C++ applications. You may want to use functionality from one application 
    in another or use functionality that is exposed through Component Object Model (COM) assemblies, such as 
    the Microsoft Word 14.0 Object Library assembly, in your applications. 

    Visual C# is a strongly typed static language. When you create a variable, you specify the type of that 
    variable, and you can only invoke methods and access members that this type defines. If you try to call 
    a method that the type does not implement, your code will not compile. This is good because the compile 
    time checks catch a large number of possible errors even before you run your code.
    
    The .NET Framework provides dynamic objects so that you can define objects that are not constrained by the 
    static type system in Visual C#. Instead, dynamic objects are not type checked until run time, which enables 
    you to write code quickly without worrying about defining every member until you need to call them in your 
    code. 
    ...
    Languages such as IronPython and IronRuby are dynamic and enable you to write code that is not compiled 
    until run time. Dynamic languages provide the following benefits: 

        • They enable faster development cycles because they do not require a build or compile process. 
        • They have increased flexibility over static languages because there are no static types to worry 
        about. 
        • They do not have a strongly typed object model to learn.

    Dynamic languages do suffer from slower execution times in comparison with compiled languages, because 
    any optimization steps that a compiler may take when compiling the code are left out of the build process. 

    Unmanaged Code 

    Visual C# is a managed language, which means that the Visual C# code you write is executed by a runtime 
    environment known as the Common Language Runtime (CLR). The CLR provides other benefits, such as memory 
    management, Code Access Security (CAS), and exception handling. Unmanaged code such as C++ executes outside 
    the CLR and in general benefits from faster execution times and being extremely flexible. 

    When you build applications by using the .NET Framework, it is a common use case to want to reuse unmanaged 
    code. Maybe you have a legacy C++ system that was implemented a decade ago, or maybe you just want to use a 
    function in the user32.dll assembly that Windows provides. The process of reusing functionality that is 
    implemented in technologies other than the .NET Framework is known as interoperability. These technologies 
    can include: 
    ...
    managing the interactions between the executing assembly and the dynamic object, for example, an object that 
    is implemented in IronPython or in a COM assembly. 

    The DLR simplifies interoperating with dynamic objects in the followings ways:

        • It defers type-safety checking for unmanaged resources until run time. Visual C# is a type-safe language 
        and, by default, the Visual C# compiler performs type-safety checking at compile time.

        Type safety relies on the compiler being able to compile or interpret each of the components that the 
        solution references. When interoperating with unmanaged components, the compile-time type-safety check is 
        not always possible. 
